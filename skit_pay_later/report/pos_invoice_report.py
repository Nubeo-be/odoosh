...
class AccountInvoice_Report(models.Model):
    ...
    def _get_payment_modes(self):
        ...
        if pos_order:
            statement_ids = pos_order.payment_ids
            if statement_ids:
                for stmt in statement_ids:
                    ids.append({'journal': stmt.payment_method_id,
                                'amount': stmt.amount,
    ...
    def _get_current_pending_invoice(self):
        ...
        order = self.env['pos.order'].search([
            ('account_move', '=', current_invoice.id)])
        if order:
            paid_amount1 = 0
            paid_amount2 = 0
            if order.payment_ids:
                paid_amount1 = sum([x.amount for x in order.payment_ids if not x.payment_method_id.is_pay_later])
            account_payment = self.env['account.payment'].sudo().search([
